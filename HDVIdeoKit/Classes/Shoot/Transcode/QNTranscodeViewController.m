...
- (void)viewDidLoad {
    ...
    [backButton setImage:[UIImage imageNamed:HDBundleImage(@"paishe/qn_icon_close")] forState:(UIControlStateNormal)];
    [backButton addTarget:self action:@selector(clickBackButton) forControlEvents:(UIControlEventTouchUpInside)];
    [self.gradientBar addSubview:backButton];
    [backButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.height.mas_equalTo(CGSizeMake(44, 44));
    ...
    [nextButton mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.size.mas_equalTo(nextButton.bounds.size);
        make.right.equalTo(self.view).offset(-16);
    }];
    
    QNVerticalButton *rotateButton = [[QNVerticalButton alloc] init];
    rotateButton.titleLabel.font = [UIFont systemFontOfSize:14];
    [rotateButton setImage:[UIImage imageNamed:HDBundleImage(@"paishe/qn_rotate_video")] forState:(UIControlStateNormal)];
    ...
    [rateButton setImage:[UIImage imageNamed:HDBundleImage(@"paishe/qn_speed")] forState:(UIControlStateNormal)];
    ...
    self.rateControl.backgroundColor = [UIColor colorWithWhite:.2 alpha:.8];
    ...
    [rateButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(CGSizeMake(44, 55));
        make.width.mas_equalTo(44);
        make.height.mas_equalTo(55);
...
- (void)setupRangeView {
    ...
    [self.assetRangeBar mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(QNAssetRangeBar.suitableHeight);
    }];
}
...
- (void)rateSegmentChange:(UISegmentedControl *)segmentControl {
    ...
    [self.rateIndicatorView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.size.mas_equalTo(self.rateIndicatorView.bounds.size);
        make.centerY.mas_equalTo(segmentControl);
        make.left.mas_equalTo(segmentControl.bounds.size.width / 5 * segmentControl.selectedSegmentIndex);
    }];
...
- (void)setupPlayer {
    ...
    self.currentAsset = self.originAsset;
//    self.player.fillMode = PLSVideoFillModeStretch;
    [self.player setItemByAsset:self.currentAsset];
    ...
    NSArray *tracks = [self.currentAsset tracksWithMediaType:AVMediaTypeVideo];
    if ([tracks count] > 0) {
        ...
        if (videoTrack.naturalSize.width > videoTrack.naturalSize.height) {
            self.player.fillMode = PLSVideoFillModePreserveAspectRatioAndFill;
        }
    }
...
- (void)clickNextButton:(UIButton *)button {
    ...
    float endtime = CMTimeGetSeconds(CMTimeSubtract(self.assetRangeBar.endTime, self.assetRangeBar.startTime));
    ...
    NSLog(@"%d--%d",[time intValue],(int)endtime);
        
    if ((int)endtime > [time intValue]) {
        return [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"剪辑视频不能超过%d秒",[time intValue]]];
    }
...
- (void)gotoEditorController:(NSURL *)url {
    ...
    [asset loadValuesAsynchronouslyForKeys:@[@"duration", @"tracks"] completionHandler:^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            HDEditorViewController *controller = [[HDEditorViewController alloc] init];
